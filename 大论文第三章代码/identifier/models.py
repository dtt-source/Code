...
#是span proposal filter 层么
    ...
    def _common_forward(self, encodings: torch.tensor, context_masks: torch.tensor, token_masks:torch.tensor, entity_masks: torch.tensor,  
       ...
       h_token.max(dim=2)[0]
        ...
        if self.use_char_lstm:
           ...
           pdb.set_trace()
           self.char_embedding(char_encoding)
           self.dropout(char_embed)
           char_masks_bool.long().sum(-1,keepdim=True)
           True)
           self.char_lstm(char_embed_packed)
           batch_first=True)
           self.pool_type)
           embeds.append(h_token_char)
       ...
       spn
        ...
        if self.use_size_embedding:
            size_embeddings = self.size_embeddings(entity_sizes) sizes
       ...
       不合理的边界偏移量置零
        offsets1[entity_spans_token[:,:,0]>=entity_spans_token[:,:,1]] = 0   右边界
        offsets1[entity_spans_token[:,:,0]<0] = 0 0
        offsets1[entity_spans_token[:,:,1]>token_count] = 0 #右边界大于最大token数
    ...
    def _spn(self, encodings, h, h_token, entity_masks, entity_masks_token, size_embeddings, entity_spans_token, token_count, token_masks_bool):
       ...
       (-1e30)
       1)
       entity_spans_pool.max(dim=2)[0]

       (-1e30)
       1)
       entity_spans_pool_token.max(dim=2)[0]
...
# outer 只调整了左边界 ， 左边界向左调整一位
       ...
       #embedding_size
       start_end_embedding_outer[:,:,1,:]
        start_end_embedding_outer = start_end_embedding_outer.view(start_end_embedding_outer.size(0),start_end_embedding_outer.size(1),-1) #view中一个参数定为-1，代表自动调整这个维度上的元素个数，以保证元素的总数不变。

# inner只调整右边界 ，右边界想左调整一位
       ...
       start_end_embedding)/2
       start_end_embedding_inner
       ...
       dim=2)
        ...
        entity_repr_outer = torch.cat(embed_outer, dim=2)0.]
    ...
    def _classify_entities(self, encodings, h, h_token, entity_masks, entity_masks_token, size_embeddings, entity_spans_token, token_count, token_masks_bool):
       ...
       entity_spans_token.clone()
       entity_spans_token_outer[:,:,0]-1
       entity_spans_token_outer[:,:,1]
       0
       1
       #embedding_size
       entity_spans_token_outer)
       #embedding_size
       start_end_embedding_outer[:,:,1,:]
       start_end_embedding_outer.view(start_end_embedding_outer.size(0),start_end_embedding_outer.size(1),-1)
       ...
       dim=2)
       dim=2)
       self.dropout(entity_repr_outer)
       ...
       self.offset_regressier2(entity_repr_outer)
        offsets = None
        return entity_clf, entity_spans_pool, offsets
