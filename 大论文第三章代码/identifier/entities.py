...
class Token:
   "SPACE"]
    def __init__(self, tid: int, index: int, span_start: int, span_end: int, phrase: str, pos: int, vocab_id: int, char_start: int, char_end: int):
        self._tid = tid dataset
        self._index = index document

        self._span_start = span_start (inclusive)
        self._span_end = span_end (exclusive)
    ...
    def __getitem__(self, index: int):
        ...
        if self._mode == Dataset.TRAIN_MODE:
            return sampling.create_train_sample(doc, self._neg_entity_count, self._neg_rel_count,
                                                self._window_size, len(self._rel_types), self.iou_min, self.iou_max,file)
        else:
