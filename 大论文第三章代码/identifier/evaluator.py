...
class Evaluator:
    def __init__(self, dataset: Dataset, input_reader: JsonInputReader, text_encoder: BertTokenizer,no_overlapping: bool,
       ...
       entities
        self._gt_entities = [] truth
        self._pred_entities = [] prediction
        self._raw_preds = []

        self._pseudo_entity_type = EntityType('Entity', 1, 'Entity', 'Entity') evaluation
        self._nms = nms
    ...
    def eval_batch(self, batch_entity_clf: torch.tensor,  spn_mask:torch.tensor, offsets1:torch.tensor, offsets2:torch.tensor,  batch: dict, illegal_mask):
       ...
       print(offsets)
        for i in range(batch_size):
           ...
           self._remove_partial_overlapping(sample_pred_entities)
    ...
    def _convert_gt(self, docs: List[Document]):
        for doc in docs:
           ...
           self._no_overlapping:
           self._remove_overlapping(sample_gt_entities)
    ...
    def _convert_example(self, doc: Document, gt: List[Tuple], pred: List[Tuple],
       ...
       scores
        if gt or pred:
            pred_s = [p[:3] for p in pred] score
       ...
       self._prettify(self._text_encoder.decode(encoding))
    ...
    def _entity_to_html(self, entity: Tuple, tokens: List[Token]):
       ...
       self._text_encoder.decode(encoding[:start])
       self._text_encoder.decode(encoding[start:end])
       self._text_encoder.decode(encoding[end:])
