...
class BiaffineScore(object):
    ...
    def compute(self, pred, labels):
        ...
        self.P.append(precision)
        self.count += 1

    def result(self):
        return {'f1': sum(self.F)/self.count, 'precision': sum(self.P)/self.count, 'recall': sum(self.R)/self.count}

    def update(self, label_paths, pred_paths):
        res = self.compute(labels=label_paths, pred=pred_paths)
