...
class WordEmbeddingAdapter(nn.Module):
    ...
    def forward(self, layer_output, word_embeddings, word_mask):
        ...
        socres = torch.matmul(layer_output.unsqueeze(2), self.attn_W)
        socres = torch.matmul(socres, torch.transpose(word_outputs, 2, 3))
        socres = socres.squeeze(2)
        socres.masked_fill_(word_mask.bool(), -1e9)
        socres = F.softmax(socres, dim=-1)
        attn = socres.unsqueeze(-1)
