...
# def create_train_sample(doc, neg_entity_count: int, neg_rel_count: int, window_sizes: list, rel_type_count: int, iou_spn:float, iou_classifier:float,file):
...
#
   ...
   'w').close()
   writer:
    with open(f"span.txt", mode='a+', errors='ignore') as writer:
        for window_size in window_sizes:
            for i in range(0, token_count):
               window_size)
                ...
                writer.write( str(doc.tokens[w_left:w_right]))
                writer.write('\n')
                writer.flush()
   ...
   tokens
    context_masks = torch.ones(context_size, dtype=torch.bool) #torch.ones:用来将tensor中元素值全置为1
   ...
   encoding
    ...
    rel_types_onehot = rel_types_onehot[:, 1:] relation
...
def create_entity_mask(start, end, context_size):
    mask = torch.zeros(context_size, dtype=torch.bool)  #返回一个形状为为size,类型为torch.dtype，里面的每一个值都是0的tensor
    mask[start:end] = 1
    return mask
